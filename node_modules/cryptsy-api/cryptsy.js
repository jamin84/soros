...
function CryptsyClient(key, secret, requeue) {
  ...
  function api_query(method, callback, args)
  {
    ...
    {
      options.method = 'GET';
      options.uri = 'http://pubapi1.cryptsy.com/api.php?' + stringify(args); 
    }
    else
    {
    ...
    request(options, function(err, res, body) {
      var response = JSON.parse(body);log.debug(response);
    ...
    */
    request(options, function(err, res, body) {
      if(!body || !res || res.statusCode != 200) {log.debug('Failed...');
        var requeue = +self.requeue;

        if(requeue) {log.debug('retrying in ', requeue, 'ms...')
          setTimeout(function() {
  ...
  self.marketorders = function(marketid, callback) {log.debug(callback);
